...
export async function categoriesRouter(request, env) {
    ...
    if (path === '/' && method === 'POST') {
        ...
        try {
            const { name, slug, description, image_url, sort_order } = await request.json();
            if (!name || !slug) return error('Name and slug are required');
            const result = await env.DB.prepare(
                'INSERT INTO categories (name, slug, description, image_url, sort_order) VALUES (?, ?, ?, ?, ?) RETURNING *'
            ).bind(name, slug.toLowerCase(), description || null, image_url || null, sort_order || 0).first();
    ...
    if (path.match(/^\/\d+$/) && method === 'PUT') {
        ...
        const { name, slug, description, image_url, sort_order, is_active } = await request.json();
        await env.DB.prepare(
            'UPDATE categories SET name=?, slug=?, description=?, image_url=?, sort_order=?, is_active=? WHERE id=?'
        ).bind(name, slug, description, image_url, sort_order, is_active ? 1 : 0, id).run();
